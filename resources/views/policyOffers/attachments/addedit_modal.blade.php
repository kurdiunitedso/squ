...
<div class="modal-content">
    ...
    <div class="modal-body scroll-y mx-5 my-7">
        ...
        <form id="kt_modal_add_attachment_form" class="form"
              ...
              action="{{ isset($attachment) ? route('policyOffers.attachments.update', ['policyOffer' => $policyOffer->id, 'attachment' => $attachment->id]) : route('policyOffers.attachments.store', ['policyOffer' => $policyOffer->id]) }}">
            <div class="d-flex flex-column scroll-y me-n7 pe-7" id="kt_modal_add_attachment_scroll"
                ...
                <!--begin::Input group-->
                @if(app()->request->attachemnt_type!='undefined')
                    <input type="hidden" value="{{\App\Models\Constant::where('module' , \App\Enums\Modules::POLICYOFFER)->where( 'field', \App\Enums\DropDownFields::ATTACHMENT_TYPE)->where('value',app()->request->attachemnt_type)->get()->first()?\App\Models\Constant::where('module' , \App\Enums\Modules::POLICYOFFER)->where( 'field', \App\Enums\DropDownFields::ATTACHMENT_TYPE)->where('value',app()->request->attachemnt_type)->get()->first()->id:"0"}}" name="attachment_type_id">
                    <h4>{{__(app()->request->attachemnt_type)}}</h4>
                @else
                    <div class="row">
                        <div class="col-md-6">
                            <!--begin::Input group-->
                ...
                @isset($attachment)
                    <div class="fv-row">
                        ...
                        <div class="mb-3">
                            ...
                            <a target="_blank" href="{{ asset('policyOffers_attachment/' . $attachment->file_hash) }}">
