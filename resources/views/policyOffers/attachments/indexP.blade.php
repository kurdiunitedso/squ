                ...
                <div class="card">
                    <div class="card-header">
                        ...
                        <div class="card-title m-0">
                            <h3 class="fw-bold m-0">{{__('PolicyOffer Attachments')}}</h3>
                        </div>
                    ...
                    <div class="card-body p-9">
                        <table class="table table-bordered align-middle table-row-dashed fs-6 gy-5"
                               id="kt_table_policyOffer_attachments">
                            <!--begin::Table head-->
                            <thead>
                            <!--begin::Table row-->
...
<script>
    var table = document.querySelector('#kt_table_policyOffer_attachments');
    ...
    var datatableAtt = $(table).DataTable({
        ...
        ajax: {
            url: "{{ route('policyOffers.attachments', ['policyOffer' => $policyOffer->id]) }}",
            type: "POST",
            data: function (d) {
                // var params = {};
...
<script>
                                ...
                                $.ajax({
                                    ...
                                    success: function (response) {
                                        ...
                                        if (response.attachment) {
                                            $(document).find('#' + response.attachment.attachment_type_id).removeClass('d-none')
                                            $(document).find('#' + response.attachment.attachment_type_id).attr('attachment', response.attachment.id)
                                        }
                                        form.reset();
                                        modal.hide();
    ...
    $(document).on('click', '.btnUpdateAttachment', function (e) {
        ...
        if ($(this).attr("attachment") != 'undefined') {
            let urlParts = currentUrl.split("/");
            var attachemnt_type = $(this).attr('attachemnt_type');
            urlParts[urlParts.length - 2] = $(this).attr("attachment")
            let newUrl = urlParts.join("/");
            currentUrl = newUrl;
        }
        renderModal(currentUrl + "?attachemnt_type=" + attachemnt_type, $(this));
    });

    $(document).on('click', '#AddAttachmentModal', function (e) {
        e.preventDefault();
        var attachemnt_type = $(this).attr('attachemnt_type');
        console.log(attachemnt_type);
        $(this).attr("data-kt-indicator", "on");
        renderModal("{{ route('policyOffers.attachments.create', ['policyOffer' => $policyOffer->id]) }}" + "?attachemnt_type=" + attachemnt_type, $(this));
    });
