<div>
    <div class="card mb-5 mb-xl-10" id="kt_claim_attachments_details_view">
        <!--begin::Card header-->
...
@push('scripts')
    <script>
        var table = document.querySelector('#kt_table_claim_attachments');
        ...
        var  datatableAtt= $(table).DataTable({
            ...
            ajax: {
                url: "{{ route('claims.attachments', ['claim' => $claim->id]) }}",
                type: "POST",
                data: function (d) {
                    // var params = {};
    ...
    <script>
        ...
        $(document).on('click', '#AddAttachmentModal', function (e) {
            e.preventDefault();
            $(this).attr("data-kt-indicator", "on");
            renderModal("{{ route('claims.attachments.create', ['claim' => $claim->id]) }}", $(this));
        });
    ...
    <script>
        $(document).on('click', '.btnDeleteClaimAttachment', function (e) {
            e.preventDefault();
