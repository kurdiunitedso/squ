@php
    ...
    $end = new DateTime($to);
 $annual=\App\Models\Constant::where('module', \App\Enums\Modules::Employee)->Where('field', \App\Enums\DropDownFields::vacation)->where('name','like', '%Annual%')->get()->first();

 $sick=\App\Models\Constant::where('module', \App\Enums\Modules::Employee)->Where('field', \App\Enums\DropDownFields::vacation)->where('name','like', '%Sick%')->get()->first();
 $approve = \App\Models\Constant::where('module', \App\Enums\Modules::Employee)->Where('field', \App\Enums\DropDownFields::vacation_status)->where('name','like', '%Approve%')->get()->first();

    $annual=isset($annual)?$annual->id:370;
     $approve=isset($approve)?$approve->id:391;
      $sick=isset($sick)?$sick->id:388;
    ...
    <div class="modal-body scroll-y mx-5 mx-xl-15 my-7">
        ...
        $hour2=date_diff( date_create($d->to_time), date_create($d->from_time))->format('%H:%I');
                                ...
                                <tr style="width: 100%;   border: 1px solid black;">
                                    ...
                                    @php
                                        if($d->last_ip!=env('CompanyIP'))
                                        $out++;
                                    @endphp
                                    <td style="border: 1px solid black">{{$d->last_ip==env('CompanyIP')?"Company":"Out Company"}}</td>
                                    ...
                                    <td style="border: 1px solid black">
                                        ...
                                        {{(\App\Models\Constant::find($d->status))?\App\Models\Constant::find($d->status)->name:'NA'}}
                        ...
                        <tr>
                            <td>
                                <strong style="color: red">
                                    ...
                                    <br>
                                    Leaves Month
                                    (Annual):{{\App\Models\EmployeeVacation::getVacationList([$annual],[$employee->id],$approve,0, $from, $end->format('Y-m-d'))->sum('days')}}
                                    <br>
                                    Leaves Month
                                    (Sick):{{\App\Models\EmployeeVacation::getVacationList([$sick],[$employee->id],$approve,0, $from, $end->format('Y-m-d'))->sum('days')}}
                                    <br>
                                    Out Company:{{$out}}
                                </strong>
