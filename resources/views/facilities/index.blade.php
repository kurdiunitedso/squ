...
@section('subpageTitle', 'Facilities')
...
@section('content')
        ...
        <div class="col-md-12 col-lg-12 col-xl-12 col-xxl-12">
            ...
            <div class="card">
                ...
                <div class="card-header border-0 pt-6">
                    ...
                    <div class="card-title">
                        ...
                        <div class="d-flex align-items-center position-relative my-1">
                            ...
                            <!--end::Svg Icon-->
                            <input type="text" data-col-index="search" data-kt-facilities-table-filter="search"
                                class="form-control datatable-input form-control-solid w-250px ps-14"
                                placeholder="Search Facilities" />
                            <input type="hidden" name="selectedFacility">
                        </div>
                        <!--end::Search-->
                    </div>
                    <!--begin::Card title-->
                    ...
                    <div class="card-toolbar">
                        <!--begin::Toolbar-->
                        <div class="d-flex justify-content-end" data-kt-facilities-table-toolbar="base">
                            <!--begin::Filter-->
                            <!--begin::facilities 1-->
                            <!--end::facilities 1-->
                            @include('facilities._filter')
                            <a href="#" id="btnAddVisitRequest" url="{{ route('visitRequests.create') }}"
                                ...
                                <span id="selectedFacilitiesRowsCount"></span>{{ __('Visit Request') }}</a>

                            <a target="_blank" id="exportBtn" href="#"
                                data-export-url="{{ route('facilities.export') }}" class="btn btn-primary me-3">
                                <i class="ki-duotone ki-exit-down fs-2"><span class="path1"></span><span
                                        class="path2"></span></i> {{ __('Export') }}
                            </a>
                            <!--end::Filter-->
                            <!--begin::Add facilities-->
                            <a href="{{ route('facilities.create') }}" class="btn btn-primary" id="AddfacilitiesModal">
                        ...
                        <div class="modal fade" id="kt_modal_add_facilities" tabindex="-1" aria-hidden="true">
                            <!--begin::Modal dialog-->
                ...
                <div class="card-body py-4">
                    <!--begin::Table-->
                    <table class="table table-bordered align-middle table-row-dashed fs-6 gy-5" id="kt_table_facilities">
                        <!--begin::Table head-->
                        <thead>
                            <!--begin::Table row-->
                            <tr class="text-start text-muted fw-bold fs-7 text-uppercase gs-0">
                                ...
                                <th class="min-w-100px bold all">{{ __('Facility ID') }}</th>
                                ...
                                <th class="min-w-100px bold all">{{ __('Has Client Trillions') }}</th>
                            ...
                            <th class="min-w-100px bold all">{{__('Beneficiary')}}</th> --}}
                                ...
                                <th class="min-w-100px bold ">{{ __('Paid to Facility') }}</th>
    ...
    <!--begin::Modal - Add task-->
    <div class="modal fade" id="kt_modal_facilities" tabindex="-1" aria-hidden="true">
        <!--begin::Modal dialog-->
...
@push('scripts')
    <script>
        var selectedFacilitiesRows = [];
        var selectedFacilitiesData = [];
        const columnDefs = [{
                data: null,
                render: function(data, type, row, meta) {
                    var isChecked = selectedFacilitiesRows.includes(row.id.toString()) ? 'checked' : '';
            ...
            {
                data: 'has_client_trillions',
                name: 'has_client_trillions',
        ...
        var datatable = createDataTable('#kt_table_facilities', columnDefs, "{{ route('facilities.index') }}", [
            [0, "DESC"]
        ]);
        ...
        $('#kt_table_facilities').find('#select-all').on('click', function() {
            $('#kt_table_facilities').find('.row-checkbox').click();
        });

        $('#kt_table_facilities tbody').on('click', '.row-checkbox', function() {
            var $row = $(this).closest('tr');
            var rowData = datatable.row($row).data();
            var rowIndex = selectedFacilitiesRows.indexOf(rowData.id);

            if (this.checked && rowIndex === -1) {
                selectedFacilitiesRows.push(rowData.id);
            } else if (!this.checked && rowIndex !== -1) {
                //console.log(data);
                selectedFacilitiesRows.splice(rowIndex, 1);

            }

            $row.toggleClass('selected');
            datatable.row($row).select(this.checked);
            if (selectedFacilitiesRows.length == 0)
                $('#selectedFacilitiesRowsCount').html("");
            else
                $('#selectedFacilitiesRowsCount').html("(" + selectedFacilitiesRows.length + ")");



            $('[name="selectedFacility"]').val(selectedFacilitiesRows.join(','));
        ...
        datatable.on('draw.dt', function() {
            datatable.rows().every(function(rowIdx, tableLoop, rowLoop) {
                ...
                if (selectedFacilitiesRows.includes(rowData.id)) {
                    this.select();
                }
            });
        });
    ...
    <script>
        const filterSearch = document.querySelector('[data-kt-facilities-table-filter="search"]');
    ...
    <script>
        ...
        const RequiredInputListFacilityChnageStatus = {}
        ...
        $(document).on('click', '.btnChangeStatus', function(e) {
            ...
            globalRenderModal(
                ...
                datatable,
                '[data-kt-changeStatus-modal-action="submit"]', RequiredInputListFacilityChnageStatus);
        });
        ...
        $(document).on('click', '.btnDeletefacility', function(e) {
            e.preventDefault();
            const URL = $(this).attr('href');
            const facilityName = $(this).attr('data-facility-name');
            Swal.fire({
                html: "Are you sure you want to delete " + facilityName + "?",
                icon: "warning",
    ...
    <script>
        $('#btnAddVisitRequest').click(function() {
            if (selectedFacilitiesRows.length == 0) {
                Swal.fire({
                    text: "Please select at least one Facility to visit Request",
                    icon: "error",
                ...
                var url = $(this).attr("url") + '?&visit_category=404&selectedFacilities=' + selectedFacilitiesRows
                    .join(',');
                renderAModal(url,
                    $(this), '#kt_modal_visits',
                    modal_kt_modal_visits,
