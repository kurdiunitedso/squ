...
@section('subpageTitle', 'Insurance Company')
            ...
            <div class="card">
                ...
                <div class="card-header border-0 pt-6">
                    ...
                    <div class="card-title">
                        ...
                        <div class="d-flex align-items-center position-relative my-1">
                            ...
                            <input type="text" data-col-index="search"
                                   data-kt-insuranceCompany-table-filter="search"
                                   class="form-control datatable-input form-control-solid w-250px ps-14"
                                   placeholder="Search InsuranceCompany"/>
                    ...
                    <div class="card-toolbar">
                        <!--begin::Toolbar-->
                        <div class="d-flex justify-content-end" data-kt-insuranceCompany-table-toolbar="base">
                            <!--begin::Filter-->
                            <!--begin::insuranceCompany 1-->
                            <!--end::insuranceCompany 1-->
                            @include('insuranceCompanys._filter')


                            <a target="_blank" id="exportBtn" href="#"
                               data-export-url="{{ route('insuranceCompanys.export') }}" class="btn btn-primary me-3">
                                <i class="ki-duotone ki-exit-down fs-2"><span class="path1"></span><span
                                        class="path2"></span></i> {{__('Export')}}
                            </a>
                            <!--end::Filter-->
                            <!--begin::Add insuranceCompany-->
                            <a href="{{ route('insuranceCompanys.create') }}" class="btn btn-primary"
                               id="AddinsuranceCompanyModal">
                            ...
                            <!--end::Add insuranceCompany-->
                        </div>
                        <!--end::Toolbar-->

                        <!--begin::Modal - Add task-->
                        <div class="modal fade" id="kt_modal_add_insuranceCompany" tabindex="-1" aria-hidden="true">
                            <!--begin::Modal dialog-->
...
@push('scripts')
    ...
    <script>
        var selectedInsuranceCompanyRows = [];
        var selectedInsuranceCompanyData = [];
            ...
            [
                {
                    ...
                    render: function (data, type, row, meta) {
                        var isChecked = selectedInsuranceCompanyRows.includes(row.id.toString()) ? 'checked' : '';
        ...
        var datatable = createDataTable('#kt_table_insuranceCompany', columnDefs, "{{ route('insuranceCompanys.index') }}", [
            [2, "DESC"]
        ]);
        ...
        $('#kt_table_insuranceCompany').find('#select-all').on('click', function () {
            $('#kt_table_insuranceCompany').find('.row-checkbox').click();
        });

        $('#kt_table_insuranceCompany tbody').on('click', '.row-checkbox', function () {
            var $row = $(this).closest('tr');
            var rowData = datatable.row($row).data();
            var rowIndex = selectedInsuranceCompanyRows.indexOf(rowData.id);

            if (this.checked && rowIndex === -1) {
                selectedInsuranceCompanyRows.push(rowData.id);
            } else if (!this.checked && rowIndex !== -1) {
                //console.log(data);
                selectedInsuranceCompanyRows.splice(rowIndex, 1);

            }

            $row.toggleClass('selected');
            datatable.row($row).select(this.checked);
            if (selectedInsuranceCompanyRows.length == 0)
                $('#selectedInsuranceCompanyRowsCount').html("");
            else
                $('#selectedInsuranceCompanyRowsCount').html("(" + selectedInsuranceCompanyRows.length + ")");


            $('[name="selectedCaptin"]').val(selectedInsuranceCompanyRows.join(','));
        ...
        datatable.on('draw.dt', function () {
            datatable.rows().every(function (rowIdx, tableLoop, rowLoop) {
                ...
                if (selectedInsuranceCompanyRows.includes(rowData.id)) {
                    this.select();
                }
            });
        });
    ...
    <script>
        const filterSearch = document.querySelector('[data-kt-insuranceCompany-table-filter="search"]');
    ...
    <script>
        $('#btnAddVisitRequest').click(function () {
            if (selectedInsuranceCompanyRows.length == 0) {
                ...
                var url = $(this).attr("url") + '?&visit_category=251&selectedInsuranceCompany=' + selectedInsuranceCompanyRows.join(',');
                renderAModal(url,
                    $(this), '#kt_modal_visits',
                    modal_kt_modal_visits,
                    [],
