        ...
        <div class="col-md-12 col-lg-12 col-xl-12 col-xxl-12">
            ...
            <div class="card">
                ...
                <div class="card-header border-0 pt-6">
                    ...
                    <div class="card-title">
                        ...
                        <div class="d-flex align-items-center position-relative my-1">
                            ...
                            <input type="text" data-col-index="search"
                                   data-kt-vehicle-table-filter="search"
                    ...
                    <div class="card-toolbar">
                        ...
                        <div class="d-flex justify-content-end" data-kt-vehicle-table-toolbar="base">
                            ...
                            <!--begin::vehicle 1-->
                            <!--end::vehicle 1-->
                            @include('vehicles._filter')


                            <a target="_blank" id="exportBtn" href="#"
                               data-export-url="{{ route('vehicles.export') }}" class="btn btn-primary me-3">
                ...
                <div class="card-body py-4">
                    <!--begin::Table-->
                    <table class="table table-bordered align-middle table-row-dashed fs-6 gy-5" id="kt_table_vehicle">
                        <!--begin::Table head-->
                        <thead>
                        <!--begin::Table row-->
                        <tr class="text-start text-muted fw-bold fs-7 text-uppercase gs-0">
                            ...
                            <th class="min-w-100px bold all">{{__('Motor CC')}}</th>
                            <th class="min-w-100px bold all">{{__('Fuel Type')}}</th>
                            <th class="min-w-100px bold all">{{__('Vehicle No')}}</th>
                            <th class="min-w-100px bold all">{{__('Box No')}}</th>
                            <th class="min-w-100px bold all">{{__('Signed promissor')}}</th>
                            <th class="min-w-100px bold all">{{__('Promissory #')}}</th>
                            <th class="min-w-100px bold all">{{__('License Expire Date')}}</th>
                            <th class="min-w-100px bold all">{{__('Has Insurance')}}</th>
                            <th class="min-w-100px bold ">{{__('Insurance Company')}}</th>
                            <th class="min-w-100px bold ">{{__('Insurance Type')}}</th>
                            <th class="min-w-100px bold ">{{__('Insurance Expire')}}</th>
...
@push('scripts')
    ...
    <script>
            ...
            [
                {
                    ...
                    render: function (data, type, row, meta) {
                        var isChecked = selectedVehicleRows.includes(row.id.toString()) ? 'checked' : '';
                ...
                },
                {
                    data: function (row, type, set) {
                        if (type === 'display') {
                            if (row.vehicle_types)
                                return row.vehicle_types.name;
                        }
                        return '';
                    },
                ...
                },
                {
                    data: function (row, type, set) {
                        if (type === 'display') {
                            if (row.vehicle_models)
                                return row.vehicle_models.name;
                        }
                        return '';
                    },
                ...
                {
                    data: 'license_expire_date2',
                    name: 'license_expire_date2',
                },
                ...
                },
                {
                    data: function (row, type, set) {
                        if (type === 'display') {
                            if (row.insurance_companys)
                                return row.insurance_companys.name;
                        }
                        return '';
                    },
                    ...
                    data: function (row, type, set) {
                        if (type === 'display') {
                            if (row.insurance_type)
                                return row.insurance_types.name;
                        }
                        return '';
                    },
                    name: 'insurance_types.name',
                },
        ...
        var datatable = createDataTable('#kt_table_vehicle', columnDefs, "{{ route('vehicles.index') }}", [
            [0, "ASC"]
        ]);
        ...
        $('#kt_table_vehicle').find('#select-all').on('click', function () {
            $('#kt_table_vehicle').find('.row-checkbox').click();
        });

        $('#kt_table_vehicle tbody').on('click', '.row-checkbox', function () {
            var $row = $(this).closest('tr');
            var rowData = datatable.row($row).data();
            var rowIndex = selectedVehicleRows.indexOf(rowData.id);

            if (this.checked && rowIndex === -1) {
                selectedVehicleRows.push(rowData.id);
            } else if (!this.checked && rowIndex !== -1) {
                //console.log(data);
                selectedVehicleRows.splice(rowIndex, 1);
            ...
            if (selectedVehicleRows.length == 0)
                $('#selectedVehicleRowsCount').html("");
            else
                $('#selectedVehicleRowsCount').html("(" + selectedVehicleRows.length + ")");


            $('[name="selectedCaptin"]').val(selectedVehicleRows.join(','));
        ...
        datatable.on('draw.dt', function () {
            datatable.rows().every(function (rowIdx, tableLoop, rowLoop) {
                ...
                if (selectedVehicleRows.includes(rowData.id)) {
                    this.select();
                }
            });
        });
    ...
    <script>
        const filterSearch = document.querySelector('[data-kt-vehicle-table-filter="search"]');
    ...
    <script>
        $('#btnAddVisitRequest').click(function () {
            if (selectedVehicleRows.length == 0) {
                ...
                var url = $(this).attr("url") + '?&visit_category=251&selectedVehicle=' + selectedVehicleRows.join(',');
                renderAModal(url,
                    $(this), '#kt_modal_visits',
                    modal_kt_modal_visits,
                    [],
