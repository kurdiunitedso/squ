    ...
    <tbody>
        ...
        @foreach ($calls as $call)
            <tr>
                ...
                <td>
                    @can('captin_call_delete')
                        <a data-call-name="register-id : {{ $call->id }}"
                            data-refresh-url={{ route('calls.captin.view_captins_calls', ['captin' => $captin->id]) }}
                            href="{{ route('calls.captin.delete', ['call' => $call->id]) }}"
                    ...
                    @if ($call->call_questionnaire_responses_count > 0)
                        <a href="{{ route('calls.captin.view_call_questionnaire_responses', ['call' => $call->id]) }}"
