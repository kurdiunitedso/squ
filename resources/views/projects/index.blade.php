...
@section('title', Project::ui['p_ucf'])
@section('subpageTitle', Project::ui['p_ucf'])
                ...
                <div class="card-header border-0 pt-6">
                    ...
                    <div class="card-title">
                        ...
                        <div class="d-flex align-items-center position-relative my-1">
                            ...
                            <input type="text" data-col-index="search" data-kt-table-filter="search"
                                ...
                                placeholder="{{ t('Search ' . Project::ui['s_ucf']) }}" />
                    ...
                    <div class="card-toolbar">
                        ...
                        <div class="d-flex justify-content-end" data-kt-departments-table-toolbar="base">
                            ...
                            <!--begin::captins 1-->
                            <!--end::captins 1-->
                            @include('departments._filter')

                            <a target="_blank" id="exportBtn" href="#"
                                data-export-url="{{ route(Project::ui['route'] . '.export') }}"
                            ...
                            <!--end::Filter-->
                            <!--begin::Add captins-->
                            <a href="{{ route(Project::ui['route'] . '.create') }}" class="btn btn-primary"
                                id="add_{{ Project::ui['s_lcf'] }}_modal">
...
@push('scripts')
    <script>
        ...
        const columnDefs = [{
            ...
            {
                data: 'contract.client_trillion.name',
                name: 'contract.client_trillion.name',
                render: function(data, type, row) {
                    const projectEditUrl = "{{ route('projects.edit', ['project' => ':id']) }}";

                    if (type === 'display') {
                        let clientName = row.contract?.client_trillion?.name ?? 'NA';
                        let url = projectEditUrl.replace(':id', row.id);

                        return `<a href="${url}" class="">
                      ${clientName}
                   </a>`;
                    }
                    return row.contract?.client_trillion?.name ?? 'NA';
                }
            },

            {
                ...
                render: function(data, type, row) {
                    return row.project_type && row.project_type.name ? row.project_type.name : 'NA';
                }
            },
            ...
            },
            {
                ...
                render: function(data, type, row) {
                    return row.account_manager && row.account_manager.name ? row.account_manager.name : 'NA';
                }
            },
            {
                data: 'art_manager.name',
                name: 'art_manager.name',
                render: function(data, type, row) {
                    return row.art_manager && row.art_manager.name ? row.art_manager.name : 'NA';
                }
            },
            {
                data: 'project_employees_count',
                name: 'project_employees_count',
                render: function(data, type, row) {
                    return row.project_employees_count ? row.project_employees_count : 'NA';
                }
            },

            {
                ...
                name: 'status.name',
                render: function(data, type, row) {
                    if (type === 'display' && data && data.color) {
                        return '<span class="badge" style="background-color: ' + data.color +
                            '; color: ' + getContrastYIQ(data.color) + ';">' +
                            (data.name || 'NA') + '</span>';
                    }
                    return data && data.name ? data.name : 'NA';
                }
            },
    ...
    <script>
        $(document).on('click', '.btnDelete' + "{{ Project::ui['s_ucf'] }}", function(e) {
            e.preventDefault();
            const URL = $(this).attr('href');
            const itemModelName = $(this).attr('data-' + "{{ Project::ui['s_ucf'] }}" + '-name');
            Swal.fire({
