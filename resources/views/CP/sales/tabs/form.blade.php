    ...
    <div class="card mb-5 mb-xl-10">
        <div class="card-body p-9">
            <div class="row">
                ...
                <div class="col-md-4">
                    <div class="fv-row mb-7">
                        ...
                        <select name="contract_type_id"
                            class="form-select form-select-solid validate-required @error('contract_type_id') is-invalid @enderror">
                            <option value="">{{ t('Select Contract Type') }}</option>
                            @foreach ($sales_contract_type_list ?? [] as $item)
                                <option value="{{ $item->id }}"
                                    {{ $_model->contract_type_id == $item->id ? 'selected' : '' }}>
                                    {{ $item->name }}
                                </option>
                            @endforeach
                        </select>
                ...
                <div class="col-md-4">
                    <div class="fv-row mb-7">
                        ...
                        <select name="payment_method_id"
                            class="form-select form-select-solid validate-required @error('payment_method_id') is-invalid @enderror">
                            <option value="">{{ t('Select Payment Method') }}</option>
                            @foreach ($sales_payment_type_list ?? [] as $item)
                                <option value="{{ $item->id }}"
                                    {{ $_model->payment_method_id == $item->id ? 'selected' : '' }}>
                                    {{ $item->name }}
                                </option>
                            @endforeach
                        </select>
                ...
                <div class="col-md-4">
                    <div class="fv-row mb-7">
                        <label class="fw-semibold fs-6 mb-2">{{ t('Deal Date') }}</label>
                        <div class="position-relative d-flex align-items-center">
                            ...
                            <input type="text" name="deal_date"
                                class="form-control form-control-solid ps-12 flatpickr-input mb-3 mb-lg-0 validate-required date-picker @error('deal_date') is-invalid @enderror"
                                placeholder="{{ t('Select Date') }}" value="{{ $_model->deal_date }}" />
                ...
                <div class="col-md-4">
                    <div class="fv-row mb-7">
                        ...
                        <input type="number" name="price"
                            ...
                            @if ($_model->exists) disabled @endif />
                        @error('price')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                    </div>
                ...
                <div class="col-md-4">
                    <div class="fv-row mb-7">
                        ...
                        <select name="status_id"
                            ...
                            @foreach ($sales_status_list ?? [] as $item)
                                <option value="{{ $item->id }}"
