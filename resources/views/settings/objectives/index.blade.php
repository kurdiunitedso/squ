...
@section('title', Objective::ui['p_ucf'])
@section('subpageTitle', Objective::ui['p_ucf'])
                    ...
                    <div class="card-title">
                        ...
                        <div class="d-flex align-items-center position-relative my-1">
                            ...
                            <input type="text" data-col-index="search" data-kt-table-filter="search"
                                ...
                                placeholder="{{ t('Search ' . Objective::ui['s_ucf']) }}" />
                    ...
                    <div class="card-toolbar">
                        ...
                        <div class="d-flex justify-content-end" data-kt-departments-table-toolbar="base">
                            ...
                            <a target="_blank" id="exportBtn" href="#"
                                data-export-url="{{ route('settings.'.Objective::ui['route'] . '.export') }}"
                            ...
                            <!--end::Filter-->
                            <!--begin::Add captins-->
                            <a href="{{ route('settings.' . Objective::ui['route'] . '.create') }}" class="btn btn-primary"
                                id="add_{{ Objective::ui['s_lcf'] }}_modal">
                    ...
                    <table class="table table-bordered align-middle table-row-dashed fs-6 gy-5" id="kt_table_items_model">
                        ...
                        <thead>
                            ...
                            <tr class="text-start text-muted fw-bold fs-7 text-uppercase gs-0">
                                ...
                                <th class="min-w-100px bold all">{{ t('Type') }}</th>
                                <th class="min-w-100px bold all">{{ t('Active') }}</th>
                                {{-- <th class="min-w-100px bold ">{{ t('Active') }}</th> --}}
...
@push('scripts')
    <script>
        ...
        const columnDefs = [
            ...
            {
                data: 'objective_type.name',
                name: 'objective_type.name',
                render: function(data, type, row) {
                    return row.objective_type && row.objective_type.name ? row.objective_type.name : 'NA';
                }
            },

            {
                ...
                name: 'active',
                render: function(data, type, row) {
                    return row.active ? row.active : 'NA';
                }
            },
        ...
        var datatable = createDataTable('#kt_table_items_model', columnDefs,
            "{{ route('settings.' . Objective::ui['route'] . '.index') }}", [
                [0, "ASC"]
            ]);
    ...
    <script>
        $(document).on('click', '.btnDelete' + "{{ Objective::ui['s_ucf'] }}", function(e) {
            e.preventDefault();
            const URL = $(this).attr('href');
            const itemModelName = $(this).attr('data-' + "{{ Objective::ui['s_ucf'] }}" + '-name');
            Swal.fire({
