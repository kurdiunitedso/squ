        ...
        <div class="fv-row mb-7">
            ...
            <select class="form-select form-select-solid mb-3 mb-lg-0" data-control="select2"
                ...
                @foreach ($BANKS as $BANK)
                    ...
                    @isset($client)
                        @selected($client->bank_name == $BANK->id)
                        @endisset>
                        {{ $BANK->name }}</option>
                @endforeach
            </select>
    ...
    <div class="col-md-3">
        <div class="fv-row mb-7">
            ...
            <input type="text" name="bank_branch"
                   ...
                   placeholder="" value="{{ isset($client) ? $client->bank_branch : '' }}"/>
            <!--end::Input-->
        </div>
    </div>
    ...
    <div class="col-md-3">
        <div class="fv-row mb-7">
            ...
            <input type="text" name="iban" class="form-control form-control-solid mb-3 mb-lg-0"
                   placeholder="" value="{{ isset($client) ? $client->iban : '' }}"/>
            <!--end::Input-->
        </div>
    </div>
        ...
        <div class="fv-row mb-7">
            ...
            <select class="form-select form-select-solid mb-3 mb-lg-0" data-control="select2"
                ...
                @foreach ($PAYMENT_TYPES as $p)
                    <option value="{{ $p->id }}"
                    @isset($client)
                        @selected($client->payment_type == $p->id)
                        @endisset>
                        {{ $p->name }}</option>
                @endforeach
            </select>
        ...
        <div class="fv-row mb-7">
            ...
            <select class="form-select form-select-solid mb-3 mb-lg-0" data-control="select2"
                ...
                @foreach ($PAYMENT_METHODS as $p)
                    <option value="{{ $p->id }}"
                    @isset($client)
                        @selected($client->payment_method == $p->id)
                        @endisset>
                        {{ $p->name }}</option>
                @endforeach
            </select>
    ...
    <div class="col-md-3">
        <div class="fv-row mb-7">
            ...
            <input type="text" name="benficiary" class="form-control form-control-solid mb-3 mb-lg-0"
                   placeholder="" value="{{ isset($client) ? $client->benficiary : '' }}"/>
            <!--end::Input-->
        </div>
    </div>
