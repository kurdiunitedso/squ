                ...
                <div class="card-header border-0 pt-6">
                    ...
                    <div class="card-toolbar">
                        ...
                        <div class="d-flex justify-content-end" data-kt-clients-table-toolbar="base">
                            ...
                            <a href="#" id="btnAddVisitRequest" url="{{route('visitRequests.create')}}" class="btn btn-info me-3">
                                ...
                                <span id="selectedClientsRowsCount"></span>{{__('Visit Request')}}</a>
                ...
                <div class="card-body py-4">
                    ...
                    <table class="table table-bordered align-middle table-row-dashed fs-6 gy-5" id="kt_table_clients">
                        ...
                        <tr class="text-start text-muted fw-bold fs-7 text-uppercase gs-0">
                            ...
                            <th class="min-w-100px bold ">{{__('Last order Box')}}</th>
                            <th class="min-w-100px bold ">{{__('Last order Bot')}}</th>
                            <th class="min-w-100px bold ">{{__('Last order Now')}}</th>
...
@push('scripts')
    ...
    <script>
        ...
        var client_call_sms_logs = document.querySelector(".client_call_sms_logs");
        var blockUI_client_call_sms_logs = new KTBlockUI(client_call_sms_logs, {
            message: '<div class="blockui-message"><span class="spinner-border text-primary"></span> Please wait...</div>',
        });
        $(document).ready(function () {
            $(document).on('click', '.ShowClientCallsSmsLogs', function (e) {
                e.preventDefault();
                const url = $(this).attr('href');
                var drawerElement = document.querySelector("#kt_drawer_client_call_sms_logs");
                var drawer = KTDrawer.getInstance(drawerElement);
                drawer.show();

                $(client_call_sms_logs).find('.card-body').html('');

                blockUI_client_call_sms_logs.block();
                ...
                $.ajax({
                    ...
                    success: function (response) {
                        // console.log(response);
                        $(client_call_sms_logs).find('.card-title span').text(response
                            .clientName);
                        $(client_call_sms_logs).find('.card-body').html(response.drawerView);
                    },
                    complete: function () {
                        blockUI_client_call_sms_logs.release();
                        // blockUI.release();
                    }
    ...
    <script>
        $('#btnAddVisitRequest').click(function () {
            if (selectedClientsRows.length == 0) {
                ...
                var url = $(this).attr("url")+'?&visit_category=250&selectedClients='+selectedClientsRows.join(',');
                renderAModal(url,
                    $(this), '#kt_modal_visits',
                    modal_kt_modal_visits,
                    [],
