    ...
    <div class="modal-body scroll-y mx-5 mx-xl-15 my-7">
        ...
        <form id="kt_modal_add_user_form" class="form" data-editMode="{{ isset($user) ? 'enabled' : 'disabled' }}"
            ...
            <div class="d-flex flex-column scroll-y me-n7 pe-7" id="kt_modal_add_user_scroll" data-kt-scroll="true"
                ...
                <div class="fv-row mb-7">
                    <!--begin::Label-->
                    <label class="required fw-semibold fs-6 mb-2">Is Employee</label>
                    <div class="form-check form-switch form-check-custom form-check-solid">
                        <input class="form-check-input" type="checkbox" value="true" id="flexSwitchDefault"
                               {{ isset($user->isemployee) ? ($user->isemployee == true ? 'checked="checked"' : '') : '' }}
                               name="isemployee"/>
                        <label class="form-check-label" for="flexSwitchDefault">
                        </label>
                    </div>
                ...
                <div class="fv-row mb-7">
                    <!--begin::Label-->
                    <label class="required fw-semibold fs-6 mb-2">Full Name ar</label>
                    <!--end::Label-->
                    <!--begin::Input-->
                    <input type="text" name="name_ar" class="form-control form-control-solid mb-3 mb-lg-0"
                           placeholder="Full name ar" value="{{ isset($user) ? $user->name_ar : '' }}"/>
                    <!--end::Input-->
                </div>
                <div class="fv-row mb-7">
                    <!--begin::Label-->
                ...
                <div class="fv-row mb-7">
                    ...
                    <select class="form-select form-select-solid mb-3 mb-lg-0" data-control="select2"
                            name="department_id" id="department_id" data-placeholder="Select an option">
                        <option></option>
                        @foreach ($departments as $t)
                            <option value="{{ $t->id }}"
                            @if (isset($user))
                                @selected($user->department_id ==$t->id)
                                @endif>
                                {{ $t->name }}</option>
                        @endforeach
                    </select>
                ...
                <div class="fv-row mb-7">
                    <!--begin::Label-->
                    <label class=" fw-semibold fs-6 mb-2">{{__('City')}}</label>
                    <!--end::Label-->
                    <!--begin::Input-->
                    <select class="form-select form-select-solid mb-3 mb-lg-0 multiple" multiple data-control="select2"
                            name="cities[]" id="cities" data-placeholder="Select an option">
                        <option></option>
                        @foreach (\App\Models\City::all() as $t)
                            <option value="{{ $t->id }}"
                            @if (isset($user))
                                @selected(in_array($t->id,explode(',',$user->cities)))
                                @endif>
                                {{ $t->name }}</option>
                        @endforeach
                    </select>
                ...
                <div class="fv-row mb-7">
                    ...
                    <input type="text" name="position" class="form-control form-control-solid mb-3 mb-lg-0"
                           placeholder="" value="{{ isset($user) ? $user->position : '' }}"/>
                    <!--end::Input-->
                </div>
