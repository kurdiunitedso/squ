<div>
    <div class="card mb-5 mb-xl-10" id="kt_offer_attachments_details_view">
        <!--begin::Card header-->
...
@push('scripts')
    <script>
        var table = document.querySelector('#kt_table_offer_attachments');
        ...
        var  datatableAtt= $(table).DataTable({
            ...
            ajax: {
                url: "{{ route('offers.attachments', ['offer' => $offer->id]) }}",
                type: "POST",
                data: function (d) {
                    // var params = {};
    ...
    <script>
        ...
        $(document).on('click', '#AddAttachmentModal', function (e) {
            e.preventDefault();
            $(this).attr("data-kt-indicator", "on");
            renderModal("{{ route('offers.attachments.create', ['offer' => $offer->id]) }}", $(this));
        });
    ...
    <script>
        $(document).on('click', '.btnDeleteOfferAttachment', function (e) {
            e.preventDefault();
