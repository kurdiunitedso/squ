...
class MarketingAgencyClientController extends Controller
{


    public function indexClient(Request $request, MarketingAgency $marketingAgency)
    {
        if ($request->isMethod('GET')) {


            return view('marketingAgencys.clients.index', [
                'marketingAgency' => $marketingAgency,
            ]);
        }
        if ($request->isMethod('POST')) {
            $clients = Client::
            select('clients.*')->wherenotnull('marketing_agency_id')->where('marketing_agency_id',$marketingAgency->id)
            ...
            return DataTables::eloquent($clients)
                ...
                ->editColumn('name', function ($client) {
                    return
                    '<a href="' . route('marketingAgencys.clients.edit', ['client' => $client->id]) . '" class="btnUpdateClient">' . $client->name . '</a>';
                })
                ...
                ->addColumn('action', function ($client) {
                    $editBtn = '<a href="' . route('marketingAgencys.clients.edit', ['client' => $client->id]) . '" class="btn btn-icon btn-active-light-primary w-30px h-30px btnUpdateClient">
                    ...
                    $removeBtn = '<a data-client-name="' . $client->address . '" href=' . route('marketingAgencys.clients.delete', ['client' => $client->id]) . ' class="btn btn-icon btn-active-light-primary w-30px h-30px btnDeleteClient"
                                >
    ...
    public function editClient(Request $request, Client $client)
    {
        ...
        $marketingAgency = MarketingAgency::find($client->marketing_agency_id);
        ...
        $createView = view('marketingAgencys.clients.addedit'
            , [
                'client' => $client,
                'marketingAgency' => $marketingAgency,
                'cities' => $cities
    ...
    public function getByTelephone(Request $request, $telephone)
    {
        ...
        $clients = Client::with('city', 'marketingAgency')->where(DB::raw('RIGHT(telephone,9)'), 'like', '%' . substr($telephone, -9) . '%')->get();
        $createView =    view('marketingAgencys.clients.getByTelephone', [
            'clientes' => $clients,
        ])->render();
        return response()->json(['createView' => $createView]);
    ...
    public function viewCalls(Request $request, Client $client)
    {
        ...
        $callsView = view('marketingAgencys.clients.viewCalls'
            , [
                'income' => $income,
                'outcome' => $outcome,
                'sms' => $sms,
