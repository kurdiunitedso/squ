...
class FacilityAttachmentController extends Controller
{
    ...
    public function index(Request $request, Facility $facility)
    {
        ...
        if ($request->isMethod('POST')) {
            $attachments = Facility::with(['attachments', 'attachments.attachmentType', 'attachments.source'])->where('id', $facility->id)->first();
            ...
            return DataTables::of($attachments->attachments)
                ...
                ->addColumn('title', function ($attachment) {
                    return '<a target="_blank" href="' . asset('facilities_attachment/' . $attachment->file_hash) . '">' . $attachment->file_name . '</a>';
                })
                ...
                ->addColumn('action', function ($attachment) use ($facility) {
                    $viewBtn = '<a target="_blank" href="' . asset('facilities_attachment/' . $attachment->file_hash) . '" class="btn btn-icon btn-active-light-primary w-30px h-30px">
                    ...
                    $editBtn = '<a href="' . route('facilities.attachments.edit', ['facility' => $facility->id, 'attachment' => $attachment->id]) . '" class="btn btn-icon btn-active-light-primary w-30px h-30px btnUpdateAttachment">
                <span class="svg-icon svg-icon-3">
                <svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                ...
                <path d="M8.82343 12.0064L8.08852 14.3348C7.8655 15.0414 8.46151 15.7366 9.19388 15.6242L11.8974 15.2092C12.4642 15.1222 12.6916 14.4278 12.2861 14.0223L9.98595 11.7221C9.61452 11.3507 8.98154 11.5055 8.82343 12.0064Z" fill="currentColor"/>
                    ...
                    $removeBtn = '<a data-attachment-name="' . $attachment->file_name . '" href=' . route('facilities.attachments.delete', ['attachment' => $attachment->id]) . ' class="btn btn-icon btn-active-light-primary w-30px h-30px btnDeleteFacilityAttachment"
                            >
    ...
    {
        $id  = null;
        $attachmentConstants = Constant::where('module', Modules::FACILITY)
            ->where('field', DropDownFields::attachment_rest_type)->get();
        $createView = view('facilities.attachments.addedit_modal', [
            'id' => $id,
            'facility' => $facility,
    ...
    {
        $request->attachment_file->store('facilities/attachments');
        ...
        $facility->attachments()->save($attachment);

        return response()->json(['status' => true, 'message' =>  'Attachment has been added successfully!']);
    }


    public function edit(Request $request, Facility $facility, Attachment $attachment)
    {
    ...
    public function update(Request $request, Facility $facility, Attachment $attachment)
    {
