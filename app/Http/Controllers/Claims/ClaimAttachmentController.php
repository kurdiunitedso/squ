...
class ClaimAttachmentController extends Controller
{
    ...
    public function index(Request $request, Claim $claim)
    {
        ...
        if ($request->isMethod('POST')) {
            $attachments = Claim::with(['attachments', 'attachments.attachmentType', 'attachments.source'])->where('id', $claim->id)->first();
            ...
            return DataTables::of($attachments->attachments)
                ->addColumn('source', function ($attachment) {
                    ...
                    $sourceModel = $attachment->source;
                    $Module = class_basename($sourceModel);
                    $link = '<a href="/'.$sourceModel.'/'.$attachment->attachable_id.'/edit">' . $Module . '-' . $attachment->attachable_id. '</a>';
                    return $link;
                })
                ->addColumn('title', function ($attachment) {
                    return '<a target="_blank" href="' . asset('claims_attachment/' . $attachment->file_hash) . '">' . $attachment->file_name . '</a>';
                })
                ...
                ->addColumn('action', function ($attachment) use ($claim) {
                    $viewBtn = '<a target="_blank" href="' . asset('claims_attachment/' . $attachment->file_hash) . '" class="btn btn-icon btn-active-light-primary w-30px h-30px">
                    ...
                    $editBtn = '<a href="' . route('claims.attachments.edit', ['claim' => $claim->id, 'attachment' => $attachment->id]) . '" class="btn btn-icon btn-active-light-primary w-30px h-30px btnUpdateAttachment">
                <span class="svg-icon svg-icon-3">
                <svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                ...
                <path d="M8.82343 12.0064L8.08852 14.3348C7.8655 15.0414 8.46151 15.7366 9.19388 15.6242L11.8974 15.2092C12.4642 15.1222 12.6916 14.4278 12.2861 14.0223L9.98595 11.7221C9.61452 11.3507 8.98154 11.5055 8.82343 12.0064Z" fill="currentColor"/>
                    ...
                    $removeBtn = '<a data-attachment-name="' . $attachment->file_name . '" href=' . route('claims.attachments.delete', ['attachment' => $attachment->id]) . ' class="btn btn-icon btn-active-light-primary w-30px h-30px btnDeleteClaimAttachment"
                            >
    ...
    {
        $id  = null;
        $attachmentConstants = Constant::where('module', Modules::CLAIM)
            ->where('field', DropDownFields::ATTACHMENT_TYPE)->get();
        $createView = view('claims.attachments.addedit_modal', [
            'id' => $id,
            'claim' => $claim,
    ...
    {
        $request->attachment_file->store('claims/attachments');
    ...
    public function edit(Request $request, Claim $claim, Attachment $attachment)
    {
    ...
    public function update(Request $request, Claim $claim, Attachment $attachment)
    {
