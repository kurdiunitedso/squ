...
class MyEmployeeWhourController extends Controller
{
    ...
    public function indexWhour(Request $request, Employee $employee)
    {
        ...
        if ($request->isMethod('GET')) {
            ...
            return view('myemployees.working_hours.index', [
                'employee' => $employee,
            ]);
        }
        if ($request->isMethod('POST')) {
            $whours = EmployeeWhour::with('status')->with('user')->where('employee_id', $employee->id);
            ...
            return DataTables::eloquent($whours)
                ...
                ->addColumn('action', function ($whour) {
                    $editBtn = '<a href="' . route('myemployees.whours.edit', ['whour' => $whour->id]) . '" class="btn btn-icon btn-active-light-primary w-30px h-30px btnUpdateWhour">
    ...
    public function editWhour(Request $request, EmployeeWhour $whour)
    {
        ...
        $createView = view('myemployees.working_hours.addedit'
            , [
                'whour' => $whour,
                'employee' => $employee,
                'statuss' => $statuss,
