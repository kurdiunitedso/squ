...
class RestaurantBranchController extends Controller
{
    ...
    public function indexBranch(Request $request, Restaurant $restaurant)
    {
        if ($request->isMethod('GET')) {


            return view('restaurants.branchs.index', [
                'restaurant' => $restaurant,
            ]);
        }
        if ($request->isMethod('POST')) {
            $branchs = RestaurantBranch::with('city')->where('restaurant_branches.restaurant_id',$restaurant->id);
            ...
            return DataTables::eloquent($branchs)
                ...
                ->addColumn('action', function ($branch) {
                    $editBtn = '<a href="' . route('restaurants.branchs.edit', ['branch' => $branch->id]) . '" class="btn btn-icon btn-active-light-primary w-30px h-30px btnUpdateBranch">
                    ...
                    $removeBtn = '<a data-branch-name="' . $branch->address . '" href=' . route('restaurants.branchs.delete', ['branch' => $branch->id]) . ' class="btn btn-icon btn-active-light-primary w-30px h-30px btnDeleteBranch"
                                >
    ...
    public function editBranch(Request $request, RestaurantBranch $branch)
    {
        ...
        $createView = view('restaurants.branchs.addedit'
            , [
                'branch' => $branch,
                'restaurant' => $restaurant,
                'cities' => $cities,
    ...
    public function deleteBranch(Request $request, RestaurantBranch $branch)
    {
        try {
            $branch->delete();
    ...
    public function storeBranch(Request $request)
    {
        ...
        if ($request->branch_id)
            $branch = RestaurantBranch::find($request->branch_id);
        if ($branch)
            $branch->update($request->all());
        else
            $branch = RestaurantBranch::create($request->all());
    ...
    public function updateBranch(Request $request)
    {
        ...
        $branch = 0;
        if ($request->branch)
            $branch = RestaurantBranch::find($request->branch);
        if ($branch)
            $branch->update($request->all());
        else
            $branch = RestaurantBranch::create($request->all());
    ...
    public function getByTelephone(Request $request, $telephone)
    {
        ...
        $branchs = RestaurantBranch::with('city', 'restaurant')->where(DB::raw('RIGHT(telephone,9)'), 'like', '%' . substr($telephone, -9) . '%')->get();
        $createView =    view('restaurants.branchs.getByTelephone', [
            'branches' => $branchs,
        ])->render();
        return response()->json(['createView' => $createView]);
    ...
    public function viewCalls(Request $request, RestaurantBranch $branch)
    {
        ...
        $callsView = view('restaurants.branchs.viewCalls'
            , [
                'income' => $income,
                'outcome' => $outcome,
                'sms' => $sms,
