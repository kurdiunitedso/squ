...
class UsersController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        if ($request->isMethod('GET')) {
            ...
            $WHEELS_BRANCHES = Constant::where('module', Modules::Customer)->where('field', DropDownFields::WHEELS_BRANCHES)->get();
            return view('user-management.users.index', compact('roles', 'WHEELS_BRANCHES'));
        }
        if ($request->isMethod('POST')) {
            $users = User::with('branch')
                ->with('roles')
                ->with('roles.permissions')->with('permissions')
    ...
    {
        $id = $role = null;
        $WHEELS_BRANCHES = Constant::where('field', DropDownFields::WHEELS_BRANCHES)->get();
        $departments = Constant::where('module', Modules::TICKET)->where('field', DropDownFields::ticket_type)->get();
    ...
    public function update(Request $request, User $user)
    {
        try {
            ...
            if ($request->cities)
                $user->cities = implode(',', $request->cities);
            ...
            if ($request->has('isemployee'))
                $user->isemployee = true;
            else $user->isemployee = false;
            ...
            if ($employee)
                $employee->update(["name" => $user->name, "user_id" => $user->id
