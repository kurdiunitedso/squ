...
class Facility extends Model implements Auditable
{
    ...
    public const ui = [
        ...
        's_ucf' => 'Facility', //uppercase first
        'p_ucf' => 'Facilities',

        's_lcf' => 'facility', //lowercase first
        'p_lcf' => 'facilities',

        '_id' => 'facility_id',
        'controller_name' => 'FacilityController',
        'image_path' => 'facilities',
    ];
    ...
    {
        return $this->belongsTo(Constant::class, 'category_id')->withDefault(['name' => 'NA']);;
    }
    ...
    {
        return $this->hasOne(Offer::class, 'facility_id');
    }
    ...
    {
        $offerBtns = $editBtn = $removeBtn = $menu = '';

        if (Auth::user()->can('facility_edit')) {
            if (Auth::user()->canAny(['facility_edit'])) {
                ...
                if (Auth::user()->can('facility_edit')) {
                    ...
                    $menu .= '
                        <div class="menu-item px-3">
                            <a href="' . route('facilities.view_attachments', ['facility' => $this->id]) . '?type=attachments" title="attachments"  class="menu-link px-3 viewCalls" >
            ...
            $this_cat = Constant::where('module', Modules::VISIT)->where('field', DropDownFields::visit_category)->where('name', 'Facility')
                ->get()->first();
            $this_cat = $this_cat ? $this_cat->id : 0;
            $creatVisit = '  <button type="button" url="' . route('visitRequests.create') . '?telephone=' . $this->telephone . '&selectedFacilities=' . $this->id . '&visit_name=' . $this->name . '&visit_category=' . $this_cat . '" class="btn btn-icon btn-active-light-primary w-30px h-30px" id="AddvisitsModal">
            ...
            $editBtn = '<a href="' . route('facilities.edit', ['facility' => $this->id]) . '" class="btn btn-icon btn-active-light-primary w-30px h-30px btnUpdatefacility">
                                        <span class="svg-icon svg-icon-3">
            ...
            $createClient = '<a href="' . route('clientTrillions.create') . '?facility=' . $this->id . '" target="_blank" class="btn btn-icon btn-active-light-primary w-30px h-30px ">
                                                <span class="svg-icon svg-icon-3">
            ...
            $createlead = '<a href="' . route('leads.create') . '?facility_id=' . $this->id . '" target="_blank" class="btn btn-icon btn-active-light-primary w-30px h-30px ">
             <span class="svg-icon svg-icon-3">
                                        <svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" width="24px" height="24px" viewBox="0 0 24 24" version="1.1">
                            <title>Lead</title>
                            <desc></desc>
            ...
            $removeBtn = '<a data-facility-name="' . $this->name . '" href=' . route('facilities.delete', ['facility' => $this->id]) . ' class="btn btn-icon btn-active-light-primary w-30px h-30px btnDeletefacility"
                    >
            ...
            $offerBtns = $this->getOfferButton($this->offer);
        }
        return $offerBtns . $editBtn . $creatVisit . $createClient . $createlead . $removeBtn . $menu;
    }


    private function getOfferButton($offer)
    {
        return $offer ? $this->getViewOfferButton($offer) : $this->getAddOfferButton();
    }

    private function getAddOfferButton()
    {
        $route = route(Offer::ui['route'] . '.create', [self::ui['_id'] => $this->id]);
        $title = t('Create ' . Offer::ui['s_lcf']);
        $class = 'btn_add_' . Offer::ui['s_lcf'];
    ...
    private function getViewOfferButton($offer)
    {
        $route = route(Offer::ui['route'] . '.edit', [Offer::ui['s_lcf'] => $offer->id]);
        $title = t('View ' . Offer::ui['s_lcf']);
