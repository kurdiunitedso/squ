...
class ExternalAppointmentExport implements FromView, WithStyles
{
    ...
    public function view($type = 0): View
    {
        ...
        $query = ExternalAppointment::with(
            'patient',
            'serviceType',
            'source',
        )->select('external_appointments.*');
        ...
        if ($patient_clinic_id != null) {
            $query->whereHas('patient', function ($patientQuery) use ($patient_clinic_id) {
                $patientQuery->where('patient_clinic_id', $patient_clinic_id);
            });
        }
        ...
        if ($appointment_date != null) {
            ...
            $query->whereBetween('appointment_date', [$date[0], $date[1]]);
        }
        ...
        $externalAppointments = $query->get();
        if ($type == 2)
            return view('externalAppointments.export2', [
                'externalAppointments' => $externalAppointments
            ]);
        return view('externalAppointments.export', [
            'externalAppointments' => $externalAppointments
        ]);
    }
